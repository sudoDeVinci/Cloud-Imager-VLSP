...
void sendStats(Network *network, Sensors::Status *stat, const String& timestamp) {
    ...
    network -> HTTP -> begin(host.toString(), 8080, String(PATH + "?" + values));
...
void OTAUpdate(Network *network, String firmware_version) {
    ...
    t_httpUpdate_return ret = httpUpdate.update(*client, host.toString(), 8080, network->routes.UPDATE, firmware_version);
    switch (ret) {
