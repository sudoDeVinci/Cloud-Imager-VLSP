...
/**
 * Toggle the heater for the SHT31-D.
 */
...
/**
 * Setup and calibrate the BMP390.
 */
...
/**
 * Setup and calibrate the SSD1306 OLED.
 */
...
void displayReadings(Reading* readings, Adafruit_SSD1306 *display) {
  ...
  display -> println(readings -> temperature + " C");

  display -> setCursor(50, 22);
  display -> println(readings -> humidity + " %");

  display -> setCursor(50, 34);
  display -> println(readings -> pressure + " Pa");

  display -> setCursor(50, 46);
  display -> println(readings -> dewpoint + " C");
...
Reading readAll(Sensors::Status *stat, Adafruit_SHT31 *sht, Adafruit_BMP3XX *bmp) {
  ...
  */
  ...
  if (temperature != UNDEFINED &&
    humidity !=UNDEFINED &&
    pressure !=UNDEFINED && 
    altitude !=UNDEFINED) readings.dewpoint = calcDP(temperature, humidity, pressure, altitude);

  if(temperature != UNDEFINED) readings.temperature = String(temperature);
  if(humidity != UNDEFINED) readings.humidity = String(humidity);
  if(pressure != UNDEFINED) readings.pressure = String(pressure);
...
String readingsToString(Reading* readings) {
  int length = readings -> timestamp.length() +
               readings -> temperature.length() +
               readings -> humidity.length() + 
               readings -> pressure.length() +
               readings -> dewpoint.length() + 4;
  
  String out;
  out.reserve(length);
  out.concat(readings -> timestamp + ",");
  out.concat(readings -> temperature + ",");
  out.concat(readings -> humidity + ",");
  out.concat(readings -> pressure + ",");
  out.concat(readings -> dewpoint);
  
  return out;
}

/**
 * Print the readings for the sensors.
 */
void printReadings(Reading* readings) {
  debug(readings -> temperature + " deg C | ");
  debug(readings -> humidity + " % | ");
  debug(readings -> pressure + " Pa | ");
  debug(readings -> dewpoint + " deg C | ");
  debugln();
}
...
Reading* csvToReadings(std::vector<String*> csv) {
  Reading* readings = new Reading[csv.size()];

  for (int i = 0; i < csv.size(); i++) {
    String* row = csv[i];
    readings[i].timestamp = row[0];
    readings[i].temperature = row[1];
    readings[i].humidity = row[2];
    readings[i].pressure = row[3];
    readings[i].dewpoint = row[4];
  }
