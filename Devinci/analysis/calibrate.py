...
def _valid_config_dict(confdict: Dict) -> bool:
    """
...
def _calibrate(calibration_image_folder: str, confdict: Dict) -> Tuple[Matlike, Matlike, Sequence[Matlike], Sequence[Matlike]] | None:
    """
...
def _write_calibration_data(cam:Camera, cameraMatrix:Matlike, dist:Matlike, rvecs:Sequence[Matlike] = None, tvecs:Sequence[Matlike] = None, timestamp: str = _current_time_as_filename()) -> None:
    """
    Write camera calibration data to toml file.
    """
    data = {'model': cam.Model.value,
            'matrix': np.asarray(cameraMatrix, dtype=np.float64).tolist(),
            'distCoeff': np.asarray(dist, dtype=np.float64).tolist(),
            'rvecs': np.asarray(rvecs, dtype=np.float64).tolist(),
            'tvecs': np.asarray(tvecs, dtype=np.float64).tolist()}

    write_toml(data, f"{cam.camera_matrices}/{timestamp}.toml")

def _undistort(img:Matlike, cameraMatrix:Matlike, dist:Matlike, remapping:bool = True, cropping:bool = True) -> Matlike:
    """
...
def _has_filetype(name: str) -> bool:
    """
...
def _filetype_match(name:str) -> str | None:
    """
...
def _load_matrix(cam: Camera, name: str) -> Dict[str, Any] | None:
    if not name.endswith('.toml'): name = f"{name}.toml"
    return load_toml(f"{cam.camera_matrices}/{name}")


def undistort(image_name: str, cam: Camera) -> None:
    """
    Undistort a given image by its name.

    Args:
        image_name (str): _description_
        cam (Camera): _description_
    ...
    if not _has_filetype(image_name):
        filetype = _filetype_match(image_name)
        if filetype is not None: image_name = f"{image_name}.{filetype}"
        else:
            debug(f"Image {image_name}.{filetype} does not exist.") 
            return None

    confdict:Dict = _load_config(config)
    if not confdict: return None
    calibration_vals = _calibrate(calibration_image_folder, confdict)
    if not calibration_vals: return None
    cameraMatrix, dist, rvecs, tvecs = calibration_vals

    timestamp = _current_time_as_filename()

    _write_calibration_data(cam, cameraMatrix, dist, rvecs, tvecs, timestamp)

    distorted = cv2.imread(f"{cam.distorted_calibration_images}/{image_name}")
    undistorted = _undistort(distorted, cameraMatrix, dist, remapping=True, cropping=False)
...
if __name__ == "__main__":
    ...
    matrix = _load_matrix(cam, timestamp)
    cmat = np.asarray(matrix['matrix'], dtype=np.float64)
    dist = np.asarray(matrix['distCoeff'], dtype = np.float64)
    rvecs = np.asarray(matrix['rvecs'], dtype = np.float64)
    tvecs = np.asarray(matrix['tvecs'], dtype = np.float64)
